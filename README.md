...
- 自动延续：进入下一周时，上一周未完成的待办会自动复制到新的一周，且同一周仅复制一次。
- 待办编辑：支持添加、修改和删除当前周待办。
- 历史查询：按日期或待办内容搜索历史周记录。
- 单周导出：仅将当前周已完成待办导出为 UTF-8 文本，每项按 `1. 2. 3.` 编号。
- 悬浮球：编辑窗口最小化后变成悬浮球；可拖动，单击展开、双击返回完整编辑界面。
- 自适应展开：面板根据悬浮球所在位置向屏幕内侧展开，项目过多时可滚动查看。
- 文字省略：悬浮展开面板中的过长任务会自动以 `…` 截断，保持列表整洁。
...
5. 点击“导出本周”选择保存位置，即可生成仅包含已完成任务的编号清单。
...
程序在打开下一周时，检查上一周的任务。未勾选的任务会以未完成状态复制到新的一周；已完成任务不会复制。程序会记录每个主周的延续操作，因此反复打开同一周不会重复添加任务。
